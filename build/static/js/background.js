...
// EXPORTS
__webpack_require__.d(__webpack_exports__, "a", function() { return /* binding */ apiReqs; });

// UNUSED EXPORTS: API_CODE, API_FAILED, apiRequest
...
var mock = __webpack_require__(68);
var mock_default = /*#__PURE__*/__webpack_require__.n(mock);

// CONCATENATED MODULE: ./src/mock.js
var domain='http://test.com/api/';// 登录
mock_default.a.mock(domain+'login',{code:200,msg:'OK',data:{nickname:'兔子先生',accessToken:'fqh0i-LyINZ-RvK5d-Akj3a-uBYRl'}});// 提交数据
mock_default.a.mock(/getData/,'get',{code:200,msg:'数据提交成功',data:{}});
// CONCATENATED MODULE: ./src/api/index.js
// 引入mock.js
// 请求服务器地址
var API_DOMAIN='http://test.com/api/';// API请求正常，数据正常
var API_CODE={// API请求正常
OK:200,// API请求正常，数据异常
ERR_DATA:403,// API请求正常，空数据
ERR_NO_DATA:301,// API请求正常，登录异常
ERR_LOGOUT:401};// API请求异常报错内容
var API_FAILED='网络连接异常，请稍后再试';// API请求汇总
var apiReqs={// 登录
signIn:function signIn(config){config.url=API_DOMAIN+'login';config.method='post';fetch(config);},// 获取数据
getData:function getData(config){config.url=API_DOMAIN+'getData';config.method='get';fetch(config);}};// 发起请求
function fetch(config){if(Object({"NODE_ENV":"production","PUBLIC_URL":"","WDS_SOCKET_HOST":undefined,"WDS_SOCKET_PATH":undefined,"WDS_SOCKET_PORT":undefined}).REACT_APP_DEBUG==='true'){// debug模式，直接发起请求
apiRequest(config);}else{// normal模式，委托background script发起请
 ...
 * config.done(): 请求结束回调
 */function apiRequest(config){// 如果没有设置config.data，则默认为{}
if(config.data===undefined){config.data={};}// 如果没有设置config.method，则默认为post
config.method=config.method||'post';// 放在请求头里的Access-Token，根据业务需求，可从localstorage里获取。演示代码暂为空。
var headers={'Access-Token':''};var data=null;// 上传文件的兼容处理，如果config.formData=true，则以multipart/form-data方式发起请求
if(config.formData){headers['Content-Type']='multipart/form-data';data=new FormData();Object.keys(config.data).forEach(function(key){data.append(key,config.data[key]);});}else{data=config.data;}// 准备好请求的全部数据
var axiosConfig={method:config.method,url:config.url,headers:headers};if(config.method==='get'){axiosConfig.params=data;}else{axiosConfig.data=data;}// 发起请求
axios_default()(axiosConfig).then(function(res){var result=res.data;// 请求结束的回调
config.done&&config.done();// 请求成功的回调
config.success&&config.success(result);}).catch(function(){// 请求结束的回调
config.done&&config.done();// 请求失败的回调
config.fail&&config.fail(API_FAILED);});}// 委托background执行请求
function sendRequestToBackground(config){config.apiType='background';if(window.chrome&&window.chrome.runtime){window.chrome&&window.chrome.runtime.sendMessage({contentRequest:'apiRequest',config:config},function(result){// 接收background script的sendResponse方法返回的消数据result
config.done&&config.done();if(result.result==='succ'){config.success&&config.success(result);}else{config.fail&&config.fail(result.msg);}});}else{console.log('未找到chrome API');}}
...
axios.isCancel = __webpack_require__(94); // Expose all/spread
...
var utils = __webpack_require__(28); // Headers whose duplicates are ignored by node
...
/* harmony import */ var _api__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(118);
var sourceUrl="https://gw.datayes-stg.com/ams_monitor_qa/web/industry/prosperity/comparison-chart/selection";var targetUrl="http://127.0.0.1:4000/ams_monitor_qa/web/industry/prosperity/comparison-chart/selection";chrome.webRequest.onBeforeSendHeaders.addListener(function(details){console.log("onBeforeSendHeaders details",details);return{requestHeaders:details.requestHeaders,redirectUrl:targetUrl};},{urls:[sourceUrl]},["blocking","requestHeaders","extraHeaders"]);
...
var buildFullPath = __webpack_require__(187);
...
module.exports = function xhrAdapter(config) {
  return new Promise(function dispatchXhrRequest(resolve, reject) {
    ...
    if (utils.isStandardBrowserEnv()) {
      var cookies = __webpack_require__(192); // Add xsrf header
